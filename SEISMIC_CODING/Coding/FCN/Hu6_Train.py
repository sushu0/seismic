...
output_image_dir = 'C:/Users/ZYH/Desktop/python_code/SEISMIC_CODING/Data/Hu6/FCN_20Hz_10'
svg_image_dir = 'C:/Users/ZYH/Desktop/python_code/SEISMIC_CODING/Data/Hu6/FCN_20Hz_10'
model_save_path = 'C:/Users/ZYH/Desktop/python_code/SEISMIC_CODING/Data/Hu6/FCN_20Hz/FCN_20Hz_model_10.pth'
norm_params_path = 'C:/Users/ZYH/Desktop/python_code/SEISMIC_CODING/Data/Hu6/FCN_20Hz/norm_params_10.json'
...
seismic_data = load_seismic_data('C:/Users/ZYH/Desktop/python_code/SEISMIC_CODING/Data/Alex/40Hz.segy')
...
# 固定测试道
# test0_idx = np.array([99, 166, 260, 415])  # 用户指定测试道号
# remaining_indices = np.setdiff1d(np.arange(500), test0_idx)
# remaining_indices = np.random.permutation(remaining_indices)


test0_idx = np.array([99, 166, 460, 415])  
train_val_indices = np.setdiff1d(np.arange(500), test0_idx)
train_idx, val_idx = train_test_split(train_val_indices, test_size=100, random_state=42)

# 显式指定测试集为预设道号
test_idx = test0_idx  # 直接使用预设道号作为测试集


# train_idx = remaining_indices[:400]
# val_idx = remaining_indices[400:450]
# test_idx = remaining_indices[450:500]
def to_tensor(data, indices):
    return torch.FloatTensor(data[indices]).unsqueeze(1)
# 转换为PyTorch张量（原有代码不变）
trainX = to_tensor(seismic_norm, train_idx)
...
try:  # 新增代码：包裹训练循环以捕获键盘中断
    for epoch in range(1):
        ...
        best_models_dir = os.path.join(model_dir, 'best_models')
        os.makedirs(best_models_dir, exist_ok=True)  # 创建最佳模型专用目录       # 修改训练循环中的模型保存逻辑
