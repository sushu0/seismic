...
output_image_dir = 'SEISMIC_CODING/output_images/True/FCN_30Hz'
svg_image_dir = 'SEISMIC_CODING/svg_images/True/FCN_30Hz'
model_save_path = 'SEISMIC_CODING/Model/True/FCN_30Hz/FCN_30Hz_model.pth'
norm_params_path = 'SEISMIC_CODING/Model/True/FCN_30Hz/norm_params.json'
...
def load_impedance_data(path):
    """加载波阻抗数据"""
    return np.loadtxt(path, usecols=4, skiprows=1).reshape(100, 20001)
...
seismic_data = load_seismic_data('SEISMIC_CODING/Data/True/30Hz.segy')
impedance_data = load_impedance_data('SEISMIC_CODING/Data/True/impedance.txt')
# 数据标准化
def normalize(data):
...
class LearnablePositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=20001):
        super().__init__()
        self.position_emb = nn.Embedding(max_len, d_model)
        
    def forward(self, x):
        # x shape: [B, C, L]
        positions = torch.arange(x.size(2), device=x.device).expand(x.size(0), x.size(2))
        pos_emb = self.position_emb(positions).permute(0, 2, 1)  # [B, C, L]
        return x + pos_emb
...
class HybridBlock(nn.Module):
    def __init__(self, in_channels, d_model=256, nhead=8):
        super().__init__()
        # Inception路径
        self.inception = InceptionModule(in_channels, d_model)
        ...
        # 维度适配
        self.channel_adapter = nn.Conv1d(d_model, d_model, 1)

    def forward(self, x):
        # Inception特征提取
        local_feat = self.inception(x)  # [B, C, L]
        
        # 转换维度适应Transformer
        B, C, L = local_feat.shape
        global_feat = local_feat.permute(2, 0, 1)  # [L, B, C]
        ...
        # 维度恢复
        global_feat = global_feat.permute(1, 2, 0)  # [B, C, L]
        global_feat = self.channel_adapter(global_feat)
...
class FCN(nn.Module):
    def __init__(self):
        ...
        # 位置编码
        self.pos_encoder = LearnablePositionalEncoding(1)
        
        # 编码器（加入Hybrid模块）
        self.enc1 = nn.Sequential(
            HybridBlock(1, d_model=64),
            nn.MaxPool1d(4)
        )
        self.enc2 = nn.Sequential(
            HybridBlock(64, d_model=128),
            nn.MaxPool1d(5)
        )
        self.enc3 = nn.Sequential(
            HybridBlock(128, d_model=256),
...
# **********************测试集可视化对比**************************
with segyio.open('SEISMIC_CODING/Data/ThreeSha/30Hz.segy', "r", ignore_geometry=True) as f_seismic:
