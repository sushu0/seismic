...
output_image_dir = 'SEISMIC_CODING/output_images/True/FCN_40Hz'
svg_image_dir = 'SEISMIC_CODING/svg_images/True/FCN_40Hz'
model_save_path = 'SEISMIC_CODING/Model/True/FCN_40Hz/FCN_40Hz_model.pth'
norm_params_path = 'SEISMIC_CODING/Model/True/FCN_40Hz/norm_params.json'
...
def load_impedance_data(path):
    """优化后的波阻抗加载，处理大文件"""
    data = np.loadtxt(path, usecols=4, skiprows=1, dtype=np.float32)
    # 内存映射处理
    mmap_file = np.memmap('impedance.dat', dtype='float32', mode='w+', shape=data.shape)
    mmap_file[:] = data[:]
    return mmap_file
...
seismic_data = load_seismic_data('SEISMIC_CODING/Data/True/40Hz.segy')
...
def normalize(data):
    ...
    # 使用原地操作减少内存占用
    data_norm = np.subtract(data, data_min, out=data.copy() if data.flags.writeable else data.copy(), dtype=np.float32)
    data_norm = np.divide(data_norm, (data_max - data_min), out=data_norm)
    return data_norm, data_min, data_max

# 在数据预处理后增加内存释放
del seismic_data, impedance_data  # 手动释放原始数据内存
...
# **********************测试集可视化对比**************************
with segyio.open('SEISMIC_CODING/Data/True/40Hz.segy', "r", ignore_geometry=True) as f_seismic:
