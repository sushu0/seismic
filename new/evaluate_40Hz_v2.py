...
print("="*50)
print("40Hz 模型评估 (直接从checkpoint加载)")
print("="*50)
...
# 直接加载完整checkpoint（包含模型对象）
ckpt = torch.load(MODEL_PATH, map_location=device, weights_only=False)

# 检查是否是完整模型或state_dict
if isinstance(ckpt, dict) and 'model' in ckpt:
    # 尝试从state_dict重建
    # 需要先从训练脚本获取模型类
    # 这里我们直接加载last.pt，可能保存方式不同
    pass

# 尝试加载last.pt
print("\n尝试加载 last.pt...")
last_model_path = 'D:/SEISMIC_CODING/new/results/01_40Hz_thinlayer/checkpoints/last.pt'
if os.path.exists(last_model_path):
    last_ckpt = torch.load(last_model_path, map_location=device, weights_only=False)
    print(f"last.pt keys: {list(last_ckpt.keys())[:5]}...")
    
# 直接执行训练脚本的模型定义部分
print("\n从训练脚本加载模型定义...")
# 读取训练脚本
with open('D:/SEISMIC_CODING/new/train_40Hz_thinlayer.py', 'r', encoding='utf-8') as f:
    script_content = f.read()

# 只执行模型定义部分（到main函数之前）
model_def_end = script_content.find('def main():')
if model_def_end > 0:
    exec_content = script_content[:model_def_end]
    exec(exec_content, globals())
    print("模型定义加载成功")
...
best_val_pcc = val_metrics.get('pcc', 'N/A') if isinstance(val_metrics, dict) else 'N/A'
print(f"加载模型 (Epoch {best_epoch}, val_pcc={best_val_pcc})")
...
test_metrics = {
    ...
    'best_val_pcc': float(best_val_pcc) if isinstance(best_val_pcc, (int, float)) else str(best_val_pcc)
}
with open(f'{OUTPUT_DIR}/test_metrics_full.json', 'w') as f:
    json.dump(test_metrics, f, indent=2)
print(f"\n保存指标到: test_metrics_full.json")
