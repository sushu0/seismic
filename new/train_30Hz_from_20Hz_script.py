...
class Config:
    ...
    # 薄层参数
    DOMINANT_FREQ = 30.0     # 主频 Hz - 修改为30Hz
    DT = 0.001               # 采样间隔 s
    TUNING_THICKNESS = 1.0 / (4 * DOMINANT_FREQ)  # 调谐厚度 ~8.3ms
...
class ForwardConsistencyLoss(nn.Module):
    ...
    def __init__(self, dominant_freq=20.0, dt=0.001):
        super().__init__()
        # 创建Ricker子波
        length = 0.1
