...
class Config:
    ...
    OUTPUT_DIR = Path(r'D:\SEISMIC_CODING\new\results\01_30Hz_thinlayer_v2_fixed')
    ...
    # 薄层参数
    DOMINANT_FREQ = 30.0     # 主频 Hz (30Hz)
    DT = 0.001               # 采样间隔 s
    TUNING_THICKNESS = 1.0 / (4 * DOMINANT_FREQ)
    
    # 数据增强
    AUGMENT_PROB = 0.5       # 薄层注入概率
    MIN_THIN_THICKNESS = 5   # 最小薄层厚度(采样点)
    MAX_THIN_THICKNESS = 30  # 最大薄层厚度(采样点)
    ...
    # 断点续训
    RESUME = False  # 从头开始训练（已修复高通滤波截止频率）
...
class ThinLayerLabeler:
    ...
    def __init__(self, dt=0.001, dominant_freq=30.0):
        self.dt = dt
...
class ThinLayerDatasetV2(Dataset):
    def __init__(self, seismic, impedance, indices, norm_stats, augmentor=None, labeler=None):
        ...
        # 降低高通滤波截止频率以保留更多有用信息（从18Hz改为12Hz）
        self.seismic_hf = highpass_filter(seismic, cutoff=12, fs=1000)
...
class ForwardConsistencyLoss(nn.Module):
    """正演一致性损失"""
    def __init__(self, dominant_freq=30.0, dt=0.001):
        super().__init__()
        length = 0.1
        ...
        for i in range(n_traces):
            ...
            for layer in layer_info:
                ...
                if matched_top is not None and matched_bottom is not None:
                    ...
                    results['dpde'].append(thickness_true)
                    results['separability'].append(0.0)
...
def main():
    ...
    # 重定向 stdout 到日志文件
    log_file = CFG.OUTPUT_DIR / 'train_log.txt'
    sys.stdout = TeeLogger(log_file, mode='a')

    print("=" * 70)
    print("ThinLayerNet V2 训练 - 30Hz 数据 - 薄层优化版")
    print("=" * 70)
    ...
    print(f"地震数据: {CFG.SEISMIC_PATH}")
    print(f"阻抗数据: {CFG.IMPEDANCE_PATH}")
    ...
    # 断点续训逻辑
    ckpt_last = CFG.OUTPUT_DIR / 'checkpoints' / 'last.pt'
    ckpt_best = CFG.OUTPUT_DIR / 'checkpoints' / 'best.pt'
    if CFG.RESUME and ckpt_last.exists():
        ...
        # 如果 last.pt 没有 best_val_pcc，尝试从 best.pt 获取
        if best_val_pcc < 0 and ckpt_best.exists():
            try:
                best_ckpt = torch.load(ckpt_best, map_location=device, weights_only=False)
                if 'val_metrics' in best_ckpt:
                    best_val_pcc = best_ckpt['val_metrics'].get('pcc', -1)
                elif 'best_val_pcc' in best_ckpt:
                    best_val_pcc = best_ckpt['best_val_pcc']
                print(f"从 best.pt 恢复 best_val_pcc: {best_val_pcc:.4f}")
            except Exception:
                pass
        print(f"从 Epoch {start_epoch} 继续训练 (best_val_pcc={best_val_pcc:.4f})\n")
    
    print(f"\n开始训练 (Epochs={start_epoch}→{CFG.EPOCHS})...")
    print("-" * 100)
    ...
    checkpoint = torch.load(CFG.OUTPUT_DIR / 'checkpoints' / 'best.pt', 
