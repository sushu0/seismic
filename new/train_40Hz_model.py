...
class Config:
    # 路径 - 40Hz数据
    SEISMIC_PATH = r'D:\SEISMIC_CODING\zmy_data\01\data\01_40Hz_re.sgy'
    IMPEDANCE_PATH = r'D:\SEISMIC_CODING\zmy_data\01\data\01_40Hz_04.txt'
    OUTPUT_DIR = Path(r'D:\SEISMIC_CODING\new\results\01_40Hz_thinlayer')
    ...
    # 薄层参数
    DOMINANT_FREQ = 40.0     # 主频 Hz - 40Hz
    DT = 0.001               # 采样间隔 s
    TUNING_THICKNESS = 1.0 / (4 * DOMINANT_FREQ)  # 调谐厚度 ~6.25ms
    
    # 数据增强
    AUGMENT_PROB = 0.5       # 薄层注入概率
    MIN_THIN_THICKNESS = 5   # 最小薄层厚度(采样点)
    MAX_THIN_THICKNESS = 25  # 最大薄层厚度(采样点) - 40Hz分辨率更高，薄层更薄
...
class ThinLayerDatasetV2(Dataset):
    def __init__(self, seismic, impedance, indices, norm_stats, augmentor=None, labeler=None):
        ...
        # 预计算高频成分 (40Hz数据用15Hz截止频率)
        self.seismic_hf = highpass_filter(seismic, cutoff=15, fs=1000)
