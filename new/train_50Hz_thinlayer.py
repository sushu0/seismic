...
class Config:
    # 路径 - 50Hz数据
    SEISMIC_PATH = r'D:\SEISMIC_CODING\zmy_data\01\data\01_50Hz_re.sgy'
    IMPEDANCE_PATH = r'D:\SEISMIC_CODING\zmy_data\01\data\01_50Hz_04.txt'
    OUTPUT_DIR = Path(r'D:\SEISMIC_CODING\new\results\01_50Hz_thinlayer')
    ...
    # 薄层参数
    DOMINANT_FREQ = 50.0     # 主频 Hz - 50Hz
    DT = 0.001               # 采样间隔 s
    TUNING_THICKNESS = 1.0 / (4 * DOMINANT_FREQ)  # 调谐厚度 ~5ms
    
    # 数据增强
    AUGMENT_PROB = 0.5       # 薄层注入概率
    MIN_THIN_THICKNESS = 4   # 最小薄层厚度(采样点)
    MAX_THIN_THICKNESS = 20  # 最大薄层厚度(采样点) - 50Hz分辨率最高，薄层更薄
...
class ThinLayerDatasetV2(Dataset):
    def __init__(self, seismic, impedance, indices, norm_stats, augmentor=None, labeler=None):
        ...
        # 预计算高频成分 (50Hz数据用20Hz截止频率)
        self.seismic_hf = highpass_filter(seismic, cutoff=20, fs=1000)
