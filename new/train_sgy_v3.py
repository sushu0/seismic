...
地震反演 v3 — 结构保持优化版
核心改进:
  1. 伪标签: 用包络+低通滤波生成, 保留地震空间结构
  2. 结构相似性损失: 反演阻抗的梯度要匹配地震梯度
  3. 更强平滑约束 + TV正则化
  4. 后处理: 更强去噪平滑
"""
...
class UNet1D(nn.Module):
    def __init__(self, in_ch=1, base=64, depth=5):
        super().__init__()
        self.depth = depth
...
def generate_pseudo_impedance_v3(seismic_raw):
    ...
    depth_trend = 5000.0 + 4000.0 * depth  # 5000 → 9000 kg/m²s

    # 5. 组合: 阻抗 = 深度趋势 + 低通分量 + 包络调制
    #    低通分量保留地震的空间纹理
    #    包络增强振幅信息
    impedance = (depth_trend
                 + 1500.0 * lowpass          # 低频地震结构
                 + 800.0 * envelope           # 振幅包络
                 )

    # 6. 强平滑: 沿道方向(sigma=5) 和横向(sigma=3)
...
def main():
    ...
    TRAIN_TRACES = 10000       # 增加训练道数
    TRACE_LEN = 512
    STRIDE = 256
    BS, EPOCHS, LR = 64, 120, 3e-4
    INFER_TRACES = 5000
    WAVELET_F0 = 25.0
    ...
    # ================== 步骤3: 模型 ==================
    P(f"\n[3/6] 构建模型...")
    model = UNet1D(in_ch=1, base=64, depth=5).to(device)
    P(f"  UNet1D(base=64, depth=5) 参数: {sum(p.numel() for p in model.parameters()):,}")
    ...
    PHYS_MAX = 0.3
    PHYS_WARMUP = 25
    STRUCT_W = 0.2      # 结构相似性权重
    TV_W = 0.05          # TV正则化权重
    SMOOTH_W = 0.08      # 平滑正则化权重
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        with torch.no_grad():
            for batch in va_ld:
                ...
                vl += mse(p, batch['imp'].to(device)).item()
                nv += 1
        ...
        if ep % 5 == 0 or ep == 1:
            P(f"  Ep {ep:3d}/{EPOCHS} | Loss: {t_losses[-1]:.5f} "
              ...
              f"| Val: {v_losses[-1]:.5f} | phys_w={lam_phys:.3f} | LR={sched.get_last_lr()[0]:.1e}")
    ...
    ax.set_title('训练损失曲线 v3', fontsize=15, fontweight='bold')
